...
const DISPLAY_TEXTURE = {false:preload("res://textures/blocks/silver_spike_up.png"),true:preload("res://textures/blocks/silver_spike_down.png")}

@onready var visible_on_screen_notifier_2d: VisibleOnScreenNotifier2D = $VisibleOnScreenNotifier2D
@onready var input_node: CanvasLayer = $Inputs

var damageEntities : Array = []
...
func calculate(inputValues := [],ignoreTick := false):
	...
	if !calculating or ignoreTick:
		...
		$DamageArea.collision_mask = 0 if on else 26
		calculating = false
...
func _on_damage_area_body_entered(body: Node2D) -> void:
	if !damageEntities.has(body):
		damageEntities.append(body)
		body.damage(3)

func _on_damage_timer_timeout() -> void:
	for body : Node2D in damageEntities:
		body.damage(3)

func _on_damage_area_body_exited(body: Node2D) -> void:
	if damageEntities.has(body):
		damageEntities.erase(body)
