...
const BUTTON_TEXTURE = {false:preload("res://textures/blocks/button_off.png"),true:preload("res://textures/blocks/button_on.png")}
...
func pressed_btn():
	if !wait:
		wait = true
		$Sprite2D.texture = BUTTON_TEXTURE[true]
		emit_signal("output","O1",true)
		await get_tree().create_timer(2).timeout
		$Sprite2D.texture = BUTTON_TEXTURE[false]
		emit_signal("output","O1",false)
		wait = false
